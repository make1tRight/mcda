...
def get_lambda_grl(ep: int, 
    ...
    p = ep / max_ep
    if mode == 'linear':
        lambda_grl = start + (end - start) * p
    elif mode == 'sigmoid':
        lambda_grl = start + (end - start) * (2. / (1. + np.exp(-10 * p)) - 1)
    else:
        raise ValueError(f"Unsupported mode: {mode}, use 'linear' or 'sigmoid'.")
    return lambda_grl
...
def mcd_train(
    ...
    for epoch in range(cfg.max_epochs):
        ...
        for batch_idx in range(len(source_loader)):
            ...
            source_features = source_features.to(cfg.device)
            source_labels = source_labels.to(cfg.device)
            ...
            # lambda_GRL = get_lambda_grl(epoch, cfg.max_epochs, mode='linear')
            features_grl = GradReverse.apply(features, cfg.lambda_GRL)
            domain_preds = model_d(features_grl)
        ...
        avg_dom_d = dom_loss_epoch / n_batches
        epoch_msg = (f"\n========== [E{epoch:03d} Summary] ==========\n"
        ...
        target_acc = evaluate_accuracy(target_loader, model_e, model_c1, model_c2, cfg, logger, "target_domain")
        logger.log("=======================================\n")
        if target_acc > best_acc:
            ...
            best_state = {
                ...
                "model_c2": model_c2.state_dict(),
            }
            logger.log("Epoch {} new best target accuracy: {:.2f}%", epoch, best_acc * 100, level="INFO")
    return best_state
